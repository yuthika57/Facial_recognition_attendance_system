import streamlit as st

st.set_page_config('Frequently Asked Questions')

st.subheader('FAQs')

def add_bg_from_url():
    st.markdown(
         f"""
         <style>
         .stApp {{
             background: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url("https://media.gettyimages.com/id/1148192366/video/hot-steel-billets-being-stacked-at-a-steel-factory.jpg?s=640x640&k=20&c=Rlrqq7hr6PCFik0kRkrXa7PXTHn9xYcS3lfuRMEAV_I=");
             background-attachment: fixed;
             background-size: cover
             
         }}
         </style>
         """,
         unsafe_allow_html=True
     )

add_bg_from_url() 


url='data:image/jpeg;base64,'

st.markdown(
        f"""
        <style>
            [data-testid="stSidebarNav"] + div {{
                position:relative;
                bottom: 0;
                height:50%;
                background-image: url({url});
                background-size: 85% auto;
                background-repeat: no-repeat;
                background-position-x: center;
                background-position-y: bottom;
            }}
        </style>
        """,
        unsafe_allow_html=True,
    )
print("Starting FAQ script")


def add_bg_from_url():
    st.markdown(
         f"""
         <style>
         .stApp {{
             background: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url("https://media.gettyimages.com/id/1148192366/video/hot-steel-billets-being-stacked-at-a-steel-factory.jpg?s=640x640&k=20&c=Rlrqq7hr6PCFik0kRkrXa7PXTHn9xYcS3lfuRMEAV_I=");
             background-attachment: fixed;
             background-size: cover
         }}
         </style>
         """,
         unsafe_allow_html=True
     )

add_bg_from_url() 

# FAQs
faqs = [
    {
        "question": "How do I enroll a new user in the attendance system?",
        "answer": '''Navigate to the Registration form section from the sidebar, Enter the name of the
        employee and select their role. Start the sample image collection process and wait for the 
        number of samples to reach 200. Once everything is collected, click on submit and wait for the 
        success message.'''
    },
    {
        "question": "How do start Real time attendance?",
        "answer": '''Navigate to the Registration form section from the sidebar,
         and click on real time prediction. Scroll down till you see camera icon and click on
         start. When you are done, click stop.'''
    },

    {
        "question": "Are my facial data and other details safe?",
        "answer": '''Data security is our utmost concern. All employee data is stored 
        securely and access to this data is restricted to Mohinder Agencies' authorized personnel only.'''
    },

    {
        "question": "How do I contact manufacturer for further queries?",
        "answer": '''You can reach out to the developer of this website at yuthikagoel9@gmail.com'''
    },
    
]


# Display FAQs using Streamlit's st.write
for i, faq in enumerate(faqs, start=1):
    st.write(f"Q{i}: {faq['question']}")
    st.write(f"A{i}: {faq['answer']}")
