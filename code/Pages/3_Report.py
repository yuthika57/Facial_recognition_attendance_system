import streamlit as st
from Home import face_rec
import pandas as pd

st.set_page_config(page_title='Reporting',layout='wide')
st.subheader('Reporting')

def add_bg_from_url():
    st.markdown(
         f"""
         <style>
         .stApp {{
             background: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url("https://media.gettyimages.com/id/1148192366/video/hot-steel-billets-being-stacked-at-a-steel-factory.jpg?s=640x640&k=20&c=Rlrqq7hr6PCFik0kRkrXa7PXTHn9xYcS3lfuRMEAV_I=");
             background-attachment: fixed;
             background-size: cover
             
         }}
         </style>
         """,
         unsafe_allow_html=True
     )

add_bg_from_url() 


url='data:image/jpeg;base64,'

st.markdown(
        f"""
        <style>
            [data-testid="stSidebarNav"] + div {{
                position:relative;
                bottom: 0;
                height:50%;
                background-image: url({url});
                background-size: 85% auto;
                background-repeat: no-repeat;
                background-position-x: center;
                background-position-y: bottom;
            }}
        </style>
        """,
        unsafe_allow_html=True,
    )

# Retrive logs data and show in Report.py
# extract data from redis list
name = 'attendance:logs'
def load_logs(name, end=-1):
    logs_list = face_rec.r.lrange(name, start=0, end=end)   #extract all data from the redis database
    return logs_list

# Bubble sort implementation
def recursive_bubble_sort(df, column_name, n=None):
    if n is None:
        n = len(df)

    # Base case: If only one element is left
    if n == 1:
        return

    # One pass of bubble sort - Move the largest element to the end
    for i in range(n - 1):
        if df[column_name][i] > df[column_name][i + 1]:
            # Swap the rows
            df.iloc[i], df.iloc[i + 1] = df.iloc[i + 1].copy(), df.iloc[i].copy()

    # Recursive call with the reduced length
    recursive_bubble_sort(df, column_name, n - 1)


# tabs to show the info

tab1, tab2 = st.tabs(["Registered Data", "Logs"])

with tab1:
        if st.button(' Refesh Data'):
            # Retrieve data from Redis data base
            with st.spinner('Retrieving data from redis db............'):
                redis_face_db = face_rec.retrive_data(name = 'company:register')
                recursive_bubble_sort(redis_face_db, 'name')
                st.dataframe(redis_face_db[['name', 'role']])


#selection sort implementation
def recursive_selection_sort(logs_df, column_name, current_index=0):
    n = len(logs_df)
    
    # Base case: If we have processed all elements
    if current_index == n - 1:
        return
    
    # Find the index of the maximum element in the unsorted part of the list
    max_index = current_index
    for i in range(current_index + 1, n):
        if logs_df[column_name][i] > logs_df[column_name][max_index]:  
            max_index = i
    
    # Swap the maximum element with the current element
    logs_df.iloc[current_index], logs_df.iloc[max_index] = logs_df.iloc[max_index].copy(), logs_df.iloc[current_index].copy()
    
    # Recursively sort the rest of the list
    recursive_selection_sort(logs_df, column_name, current_index + 1)



with tab2:
    if st.button('Refresh Logs'):
        # Load log data into logs_list 
        logs_list = load_logs(name=name)  
        
        # Split the logs into columns (name, role, date_time)
        logs_data = [log.decode('utf-8').split('@') for log in logs_list]
        
        # Create a DataFrame from the split data
        logs_df = pd.DataFrame(logs_data, columns=['Name', 'Role', 'Date and Time'])
        
        # Save the logs DataFrame to an Excel file
        excel_file = "logs_data.xlsx"
        logs_df.to_excel(excel_file, index=False)
        st.write(f"Logs saved to {excel_file}")
  
        # Display the sorted logs as a DataFrame
        recursive_selection_sort(logs_df,'Date and Time')
        st.dataframe(logs_df)

